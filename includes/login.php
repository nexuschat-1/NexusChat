<?php
    ...
    try{
        if ($_SERVER["REQUEST_METHOD"] === "POST"){
            ...
            if (empty($errors)) {
                $logs = mysqli_query($con,"SELECT pass_word, user_id FROM users WHERE username='$username'");
                if ($row = mysqli_fetch_assoc($logs)){
                    if($password == $row['pass_word']){
                        $_SESSION['user_id'] = $row['user_id'];
                        echo "<script>window.location.href = 'nexuschat.php';</script>";
                        exit();
                    }else{
                        ...
                        echo "<script>document.getElementById('cardElement').classList.toggle('flipped');</script>";
                    }
                }
            }else {
                echo "<div id='error_msg'>
