$msg_id = $DATA_OBJ->msg_id;
            $result = mysqli_query($con,"SELECT * FROM message WHERE ( sender = $user AND 	reciver = $now_chatting_with OR sender = $now_chatting_with AND reciver = $user ) AND msg_id > $msg_id  UNION  SELECT * FROM message WHERE ( sender = $user AND reciver = $now_chatting_with ) AND seen  = 0");
            if($result){
            ...
            $result = mysqli_query($con,"SELECT * FROM message WHERE reciver = $user AND sender <> $now_chatting_with AND seen  = 1");
            if($result){
                while ($row = mysqli_fetch_assoc($result)) {
                ...
                if(count($notification) != 0){
                    $chat_message->type = "notification";
                    $chat_message->notification = $notification;
                    $chat_message->final_chat_id = $reffernce_msg_id;
                    echo json_encode($chat_message);
                }
            }
